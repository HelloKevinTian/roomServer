...
Connector.prototype.createTcpServer = function() {
	...
	self.server.on('connection', function(sock) {
		...
		sock.sendMessage = function(data) {
			...
			logger.info('<<<<<< 发到客户端的数据:', sock.id, (4 + len), data);
		}
		...
		function onSocketErr() {
			...
			sock.on('close', function(had_error) {
				if (self.disconnect) {
					self.disconnect(sock, socketId, had_error);
				}
				if (!sock.destroyed) {
					sock.destroy();
				}
			...
			sock.on('c_close', function() {
				logger.error("socket c_close :");
				if (self.disconnect) {
					self.disconnect(sock, socketId);
				}
				sock.end();
				sock.destroy();
			});
	...
	try {
		...
		self.verifyMessage(args, function(result, data) {
			...
			if (!args.op || typeof args.op !== 'string') {
				return logger.error('数据缺失op字段!', args);
			}

			if (self.handler) {
				self.handler.trigger(args.op, socket.remoteAddress, data, socket);
			} else {
